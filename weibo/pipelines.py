...
class CsvPipeline(object):
    def open_spider(self, spider):
        self.save_dir = spider.settings.get('SAVE_DIR', '.')

    def process_item(self, item, spider):
        base_dir = os.path.join(self.save_dir, '结果文件', item['keyword'])
        if not os.path.isdir(base_dir):
            os.makedirs(base_dir)
        file_path = os.path.join(base_dir, item['keyword'] + '.csv')
...
class SQLitePipeline(object):
    def open_spider(self, spider):
        try:
            import sqlite3
            save_dir = spider.settings.get('SAVE_DIR', '.')
            base_dir = os.path.join(save_dir, '结果文件')
            if not os.path.isdir(base_dir):
                os.makedirs(base_dir)
            db_name = spider.settings.get('SQLITE_DATABASE', 'weibo.db')
...
class MyImagesPipeline(ImagesPipeline):
    ...
    def file_path(self, request, response=None, info=None):
        ...
        save_dir = self.crawler.settings.get('SAVE_DIR', '.')
        base_dir = os.path.join(save_dir, '结果文件', item['keyword'], 'images')
        if not os.path.isdir(base_dir):
            os.makedirs(base_dir)
...
class MyVideoPipeline(FilesPipeline):
    ...
    def file_path(self, request, response=None, info=None):
        item = request.meta['item']
        save_dir = self.crawler.settings.get('SAVE_DIR', '.')
        base_dir = os.path.join(save_dir, '结果文件', item['keyword'], 'videos')
        if not os.path.isdir(base_dir):
            os.makedirs(base_dir)
...
class MysqlPipeline(object):
    def create_database(self, mysql_config, db_name):
        import pymysql
    ...
    def open_spider(self, spider):
        try:
            ...
            db_name = cfg.get('MYSQL_DATABASE', 'weibo')
            self.create_database(mysql_config, db_name)
